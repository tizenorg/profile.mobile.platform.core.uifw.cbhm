...
static Eina_Bool __wl_selection_offer(void *data EINA_UNUSED,
...
static Eina_Bool __wl_selection_send(void *udata, int type EINA_UNUSED,
		void *event)
{
	FN_CALL();
	char *buf;
	...
	while (len_written < item->len) {
		...
		if (ret == -1)
			break;
		DBG("== [ TRANSACTION-SEND ] ====>> data(%s), len(%d)", buf, SAFE_STRLEN(buf));
...
static Eina_Bool __wl_selection_receive(void *udata, int type EINA_UNUSED,
		void *event)
{
	FN_CALL();
	int i = -1;
...
int cbhmd_wl_handler_init(cbhmd_app_data_s *ad)
{
	FN_CALL();

	RETV_IF(NULL == ad, CBHM_ERROR_INVALID_PARAMETER);

	cbhmd_wl_handler_data_s *wld = CALLOC(1, sizeof(cbhmd_wl_handler_data_s));
	if (!wld) {
		ERR("CALLOC() Fail");
