...
static char* __convert_html_to_entry_cb(Cbhmd_App_Data *ad, int type_index,
                                        const char *str);
static char* __convert_efl_to_entry_cb(Cbhmd_App_Data *ad, int type_index,
                                       const char *str);
static char* __convert_text_to_entry_cb(Cbhmd_App_Data *ad, int type_index,
                                        const char *str);
static char* __convert_image_path_to_entry_cb(Cbhmd_App_Data *ad,
                                              int type_index, const char *str);
static char* __convert_polaris_to_entry_cb(Cbhmd_App_Data *ad, int type_index,
                                           const char *str);

//static char* __convert_make_close_tag_cb(Eina_List* nodes);
char* __convert_entry_emoticon_to_normal_text(const char *src_text);
static char* __convert_do_not_convert(Cbhmd_App_Data *ad, int type_index,
                                      const char *str);
static char* __convert_html_to_efl(Cbhmd_App_Data *ad, int type_index,
                                   const char *str);
static char* __convert_efl_to_html(Cbhmd_App_Data *ad, int type_index,
                                   const char *str);
static char* __convert_text_to_html(Cbhmd_App_Data *ad, int type_index,
                                    const char *str);
static char* __convert_text_to_efl(Cbhmd_App_Data *ad, int type_index,
                                   const char *str);
static char* __convert_to_text(Cbhmd_App_Data *ad, int type_index,
                               const char *str);
static char* __convert_image_path_to_html(Cbhmd_App_Data *ad, int type_index,
                                          const char *str);
static char* __convert_image_path_to_efl(Cbhmd_App_Data *ad, int type_index,
                                         const char *str);
//static char* image_path_to_text(Cbhmd_App_Data *ad, int type_index, const char *str);
//static char* __convert_efl_to_efl_cb(Cbhmd_App_Data *ad, int type_index, const char *str);
//static char* __convert_html_to_html_cb(Cbhmd_App_Data *ad, int type_index, const char *str);
static char* __convert_image_path_to_image_path(Cbhmd_App_Data *ad,
                                                int type_index, const char *str);
static char* __convert_html_to_image_path(Cbhmd_App_Data *ad, int type_index,
                                          const char *str);
static char* __convert_efl_to_image_path(Cbhmd_App_Data *ad, int type_index,
                                         const char *str);
   ...
   if (!file || item->img_from_web || item->img_from_markup)
     {
        if (item_type_index == ATOM_INDEX_HTML)
          ...
          if (item_type_index == ATOM_INDEX_EFL)
            ad->targetAtoms[item_type_index].convert_to_target[ATOM_INDEX_IMAGE] =
               NULL;
     }
   ...
   if (!file)
     {
        if (item_type_index == ATOM_INDEX_HTML)
          ad->targetAtoms[item_type_index].convert_to_target[ATOM_INDEX_IMAGE] =
             __convert_html_to_image_path;
        else
          if (item_type_index == ATOM_INDEX_EFL)
            ad->targetAtoms[item_type_index].convert_to_target[ATOM_INDEX_IMAGE] =
               __convert_efl_to_image_path;
     }
   ...
   if (ad->targetAtoms[item_type_index].convert_to_target[req_index])
     {
        *data_ret = ad->targetAtoms[item_type_index].convert_to_target[req_index](
        ...
        if (!*data_ret)
          return EINA_FALSE;
        if (size_ret)
          *size_ret = SAFE_STRLEN(*data_ret);
        if (ttype)
          *ttype = ad->targetAtoms[item_type_index].atom[0];
        if (tsize)
          *tsize = 8;
        return EINA_TRUE;
     }
...
static void _link_match_tags(Eina_List *nodes)
{
   ...
   EINA_LIST_FOREACH(nodes, l, trail)
     {
        if (!trail || !trail->tag || trail->tag[0] == '\0')
          continue;
        ...
        if (!SAFE_STRCMP("br", trail->tag) || !SAFE_STRCMP("br/", trail->tag))
          ...
          if (!SAFE_STRCMP("item", trail->tag) || !SAFE_STRCMP("img", trail->tag))
            {
               trail->tagPosType = TAGPOS_ALONE;
               continue;
            }
        ...
        if (trail->tag[0] != '/') // PUSH
        ...
        else // POP
          {
             ...
             EINA_LIST_REVERSE_FOREACH(stack, r, popData)
               {
                  if (!popData || !popData->tag || popData->tag[0] == '\0')
                    continue;
                       ...
                       if (trail->tagPosType != TAGPOS_END)
                         {
                            if (!SAFE_STRCMP(_EFLtoHTMLConvertTable[j].src, "font"))
                              ...
                              if (!SAFE_STRCMP(_EFLtoHTMLConvertTable[j].src, "item"))
                                {
                                   PItemTagData data = trail->tagData;
                                   if (data->href)
                       ...
                       if (trail->tagPosType != TAGPOS_END)
                         {
                            if (!SAFE_STRCMP(_HTMLtoEFLConvertTable[j].src, "font"))
                              ...
                              if (!SAFE_STRCMP(_HTMLtoEFLConvertTable[j].src, "img"))
                                {
                                   PItemTagData data = trail->tagData;
...
/*
   static char* __convert_efl_to_efl_cb(Cbhmd_App_Data *ad, int type_index,
   const char *str)
   {
   FN_CALL();
   return NULL;
   ...
   static char* __convert_html_to_html_cb(Cbhmd_App_Data *ad, int type_index,
   const char *str)
   {
   FN_CALL();
   return NULL;
...
static char* __convert_markup_to_entry(Cbhmd_App_Data *ad, int type_index,
                                       const char *str)
{
   FN_CALL();
...
void cbhmd_convert_target_atoms_init(Cbhmd_App_Data *ad)
{
   ...
   char *targetAtomNames[][5] = { {"TARGETS"}, {"UTF8_STRING", "STRING", "TEXT",
        "text/plain;charset=utf-8", "text/plain"}, {"text/html;charset=utf-8",
             "text/html"}, {"application/x-elementary-markup"}, {"text/uri",
                  "text/uri-list"}, {"polaris-markup"}};

   text_converter_func converts_to_entry[ATOM_INDEX_MAX] = {NULL,
        __convert_text_to_entry_cb, __convert_html_to_entry_cb,
        __convert_efl_to_entry_cb, __convert_image_path_to_entry_cb,
        __convert_polaris_to_entry_cb};

   text_converter_func converts[ATOM_INDEX_MAX][ATOM_INDEX_MAX] = { {NULL, NULL,
        NULL, NULL, NULL, NULL}, {NULL, __convert_do_not_convert,
             __convert_text_to_html, __convert_text_to_efl, NULL, NULL}, {NULL,
                  __convert_to_text, __convert_do_not_convert, __convert_html_to_efl,
                  __convert_html_to_image_path, NULL}, {NULL, __convert_to_text,
                       __convert_efl_to_html, __convert_do_not_convert,
                       __convert_do_not_convert, NULL}, {NULL, NULL,
                            __convert_image_path_to_html, __convert_image_path_to_efl,
                            __convert_image_path_to_image_path, NULL}, {NULL, __convert_to_text,
                                 NULL, NULL, NULL, __convert_do_not_convert}};
