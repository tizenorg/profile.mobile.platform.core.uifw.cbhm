...
static Eina_Bool item_write(Eet_File *ef, int index, Cbhmd_Cnp_Item *item);
static Eina_Bool item_delete(Eet_File *ef, int index);
static Eina_Bool storage_index_write(Cbhmd_Storage_Data *sd, int index);
static Eina_Bool storage_item_write(Cbhmd_App_Data *ad, Cbhmd_Cnp_Item *item);
static Eina_Bool storage_item_delete(Cbhmd_App_Data *ad, Cbhmd_Cnp_Item *item);
static Eina_Bool storage_item_update(Cbhmd_App_Data *ad, Cbhmd_Cnp_Item *item);
static Cbhmd_Cnp_Item *storage_item_load(Cbhmd_Storage_Data *sd, int index);
//static void dump_items(Cbhmd_Storage_Data *sd);
             ...
             for (i = index - 1; i >= 0; i--)
               {
                  j = index_order[i];
                  if (sd->itemTable[j])
                    item_add_by_Cbhmd_Cnp_Item(ad, sd->itemTable[j], EINA_FALSE,
                                               EINA_FALSE);
               }
          }
        else
...
/*
   ...
   for (i = 0; i < ITEM_CNT_MAX; i++)
   {
   Cbhmd_Cnp_Item *item = storage_item_load(sd, i);
   if (item)
