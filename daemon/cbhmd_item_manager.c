...
static void item_free(Cbhmd_Cnp_Item *item, Eina_Bool storage)
{
   ...
   if (item->ad)
     {
        if (item->ad->draw_item_del) item->ad->draw_item_del(item->ad, item);
                                           ...
                                           Cbhmd_Cnp_Item *item, Eina_Bool storage, Eina_Bool show_msg)
{
   ...
   if (ad && ad->draw_item_add) duplicated = ad->draw_item_add(ad, item);
   if (storage && ad && ad->storage_item_add) ad->storage_item_add(ad, item);
...
#endif
{
   ...
   if (item->type_index == ATOM_INDEX_TEXT)
   ...
   else if (item->type_index == ATOM_INDEX_HTML)
     {
        ...
        else if (copied_path) copied_path = html_img_save_frm_local(copied_path);
   ...
   while (ad->item_list)
     {
        ...
        ad->item_list = eina_list_remove(ad->item_list, item);
        if (item) item_free(item, EINA_FALSE);
     }
}
