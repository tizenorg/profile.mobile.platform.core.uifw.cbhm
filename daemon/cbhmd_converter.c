...
Entry_Emoticon cbhmd_emotion_name_table[ENTRY_EMOTICON_MAX] = {
...
#define FREE_N_ASSIGN(dst, value) \
   do\
{\
   if (value)\
     {\
...
static void _cleanup_tag_list(Eina_List *nodeList);
...
static void _dump_node(Eina_List* nodes);

char *
cbhmd_convert_get_entry_string(Cbhmd_App_Data *ad, int type_index,
                               const char *str)
{
   DBG("type_index(%d), str(%s)", type_index, str);
   if (ad->target_atoms[type_index].convert_to_entry)
     return ad->target_atoms[type_index].convert_to_entry(ad, type_index, str);

   return NULL;
}

char *
cbhmd_convert_get_image_path_string(Cbhmd_App_Data *ad, int type_index,
                                    const char *str)
{
   DBG("type_index: %d str: %s ", type_index, str);
...
#ifdef HAVE_X11
int
atom_type_index_get(Cbhmd_App_Data *ad, Ecore_X_Atom atom)
#else
int
atom_type_index_get(Cbhmd_App_Data *ad, int atom)
#endif
{
   int i, j;
   for (i = 0; i < ATOM_INDEX_MAX; i++)
     {
        for (j = 0; j < ad->target_atoms[i].atom_cnt; j++)
          if (ad->target_atoms[i].atom[j] == atom) return i;
     }
   return -1;
}
...
#ifdef HAVE_X11
static Eina_Bool
targets_converter(Cbhmd_App_Data *ad, Ecore_X_Atom reqAtom, Cbhmd_Cnp_Item *item, void **data_ret, int *size_ret, Ecore_X_Atom *ttype, int *tsize)
#else
static Eina_Bool
targets_converter(Cbhmd_App_Data *ad, int reqAtom,
...
#endif
{
   ...
   if (!file || item->img_from_web || item->img_from_markup)
     {
        if (item_type_index == ATOM_INDEX_HTML)
          ad->target_atoms[item_type_index].convert_to_target[ATOM_INDEX_IMAGE] =
             NULL;
   ...
   for (i = 0, count = 0; i < ATOM_INDEX_MAX; i++)
     {
        if (ad->target_atoms[item_type_index].convert_to_target[i])
          count += ad->target_atoms[i].atom_cnt;
     }
        ...
        if (ad->target_atoms[item_type_index].convert_to_target[i])
          {
             for (j = 0; j < ad->target_atoms[i].atom_cnt; j++)
               {
#ifdef HAVE_X11
                  ((Ecore_X_Atom *)*data_ret)[count++] = ad->target_atoms[i].atom[j];
#else
                  ((int *)*data_ret)[count++] = ad->target_atoms[i].atom[j];
#endif
                  DBG("send target atom: %s", ad->target_atoms[i].name[j]);
               }
          }
     }
...
#else
Eina_Bool
generic_converter(Cbhmd_App_Data *ad, int reqAtom,
                  Cbhmd_Cnp_Item *item, void **data_ret, int *size_ret, int *ttype,
                  int *tsize)
#endif
{
   ...
   if (ad->target_atoms[ATOM_INDEX_TARGET].atom[0] == reqAtom)
   ...
   if (ad->target_atoms[item_type_index].convert_to_target[req_index])
     {
        *data_ret = ad->target_atoms[item_type_index].convert_to_target[req_index](
           ad, item_type_index, item_data);
        if (!*data_ret) return EINA_FALSE;
        if (size_ret) *size_ret = SAFE_STRLEN(*data_ret);
        if (ttype) *ttype = ad->target_atoms[item_type_index].atom[0];
        if (tsize) *tsize = 8;
        return EINA_TRUE;
     }
...
static PTagNode
_new_tag_node(char *tag, char *tag_str, char* str,
              const char *pos_in_ori_str)
{
                  ...
                  if (!SAFE_STRCMP("font", node->tag))
                    {
                       ...
                       SAFE_FREE(data->name);
                       SAFE_FREE(data->color);
                       SAFE_FREE(data->size);
                       SAFE_FREE(data->bg_color);
                    }
                  if (!SAFE_STRCMP("item", node->tag))
                    {
                       PItemTagData data = node->tagData;
                       SAFE_FREE(data->href);
                       SAFE_FREE(data->width);
                       SAFE_FREE(data->height);
                    }
               }
             SAFE_FREE(node->tagData);
          }
        SAFE_FREE(node->tag);
        SAFE_FREE(node);
     }
...
_set_EFL_font_data(PFontTagData data, const char *tag_str)
{
   ...
   FREE_N_ASSIGN(data->size, value);
   value = _get_tag_value(tag_str, "color");
   FREE_N_ASSIGN(data->color, value);
   value = _get_tag_value(tag_str, "bgcolor");
   FREE_N_ASSIGN(data->bg_color, value);
   value = _get_tag_value(tag_str, "font");
   FREE_N_ASSIGN(data->name, value);
        ...
        if (xpos)
          {
             ...
             FREE_N_ASSIGN(data->width, SAFE_STRNDUP(value, xpos - value));
             FREE_N_ASSIGN(data->height,
...
_set_HTML_img_data(PItemTagData data, const char *tag_str)
{
   ...
   FREE_N_ASSIGN(data->width, value);
   value = _get_tag_value(tag_str, "height");
   FREE_N_ASSIGN(data->height, value);
   return data;
}
...
static char *
_convert_do_not_convert(Cbhmd_App_Data *ad, int type_index,
                        const char *str)
{
   DBG("str(%s)", str);
   if (type_index != ATOM_INDEX_HTML)
...
static char *
_make_image_path_tag(int type_index, const char *str)
{
...
/*
   ...
   {
   DBG("str(%s)", str);
   return _make_image_path_tag(ATOM_INDEX_TEXT, str);
   }
 */

static char *
_convert_image_path_to_html(Cbhmd_App_Data *ad, int type_index,
                            const char *str)
{
   DBG("str(%s)", str);
   return _make_image_path_tag(ATOM_INDEX_HTML, str);
}

static char *
_convert_image_path_to_efl(Cbhmd_App_Data *ad, int type_index,
                           const char *str)
{
   DBG("str(%s)", str);
   return _make_image_path_tag(ATOM_INDEX_EFL, str);
}

static char *
_convert_image_path_to_image_path(Cbhmd_App_Data *ad,
                                  int type_index, const char *str)
{
   DBG("str(%s)", str);
   return _make_image_path_tag(ATOM_INDEX_IMAGE, str);
}

static char *
_convert_html_to_image_path(Cbhmd_App_Data *ad, int type_index,
                            const char *str)
{
   DBG("str(%s)", str);
...
static char *
_convert_efl_to_image_path(Cbhmd_App_Data *ad, int type_index,
                           const char *str)
{
   DBG("str(%s)", str);
   Eina_Strbuf *sbuf = eina_strbuf_new();
...
static char *
_convert_markup_to_entry(Cbhmd_App_Data *ad, int type_index,
                         const char *str)
{
   FN_CALL();
...
static char *
_convert_polaris_to_entry_cb(Cbhmd_App_Data *ad, int type_index,
                             const char *str)
{
   DBG("str(%s)", str);
...
static char *
_convert_html_to_entry_cb(Cbhmd_App_Data *ad, int type_index,
                          const char *str)
{
   DBG("str(%s)", str);
   return _convert_markup_to_entry(ad, type_index, str);
...
static int
entry_emoticon_origin_string(char **src_text, int* item_length)
{
   int i = 0;
...
char*
_convert_entry_emoticon_to_normal_text(const char *src_text)
{
...
static char *
_convert_efl_to_entry_cb(Cbhmd_App_Data *ad, int type_index,
                         const char *str)
{
   DBG("str(%s)", str);
...
static char *
_convert_image_path_to_entry_cb(Cbhmd_App_Data *ad, int type_index,
                                const char *str)
{
   FN_CALL();
   return NULL;
...
static char *
_convert_text_to_entry_cb(Cbhmd_App_Data *ad, int type_index,
                          const char *str)
{
   DBG("str(%s)", str);
   char *markup = NULL;
...
static Eina_List*
make_tag_list(int type_index, const char *str)
{
...
static void
_cleanup_tag_list(Eina_List *nodeList)
{
   Eina_List *trail;
   PTagNode nodeData;
...
static char *
_convert_html_to_efl(Cbhmd_App_Data *ad, int type_index,
                     const char *str)
{
   FN_CALL();
   Eina_List *nodeList = NULL;
   ...
   DBG("efl: %s", ret);
   _cleanup_tag_list(nodeList);

   return ret;
}

static char *
_convert_efl_to_html(Cbhmd_App_Data *ad, int type_index,
                     const char *str)
{
   FN_CALL();
   Eina_List *nodeList = NULL;
...
static char *
_convert_text_to_html(Cbhmd_App_Data *ad, int type_index,
                      const char *str)
{
   DBG("str(%s)", str);
   char *markup = NULL;
...
static char *
_convert_text_to_efl(Cbhmd_App_Data *ad, int type_index,
                     const char *str)
{
   DBG("str(%s)", str);
   char *ret = NULL;
...
static char *
_convert_to_text(Cbhmd_App_Data *ad, int type_index,
                 const char *str)
{
   DBG("str(%s)", str);
...
void
cbhmd_convert_target_atoms_init(Cbhmd_App_Data *ad)
{
   int i, j;
   ...
   for (i = 0; i < ATOM_INDEX_MAX; i++)
     {
        ad->target_atoms[i].atom_cnt = atom_cnt[i];
        ad->target_atoms[i].name = MALLOC(sizeof(char *) * atom_cnt[i]);
#ifdef HAVE_X11
        ad->target_atoms[i].atom = MALLOC(sizeof(Ecore_X_Atom) * atom_cnt[i]);
#else
        ad->target_atoms[i].atom = MALLOC(sizeof(int) * atom_cnt[i]);
#endif
        for (j = 0; j < atom_cnt[i]; j++)
          {
             DBG("atomName(%s)", targetAtomNames[i][j]);
             ad->target_atoms[i].name[j] = SAFE_STRDUP(targetAtomNames[i][j]);
#ifdef HAVE_X11
             ad->target_atoms[i].atom[j] = ecore_x_atom_get(targetAtomNames[i][j]);
#endif
          }

        ad->target_atoms[i].convert_to_entry = converts_to_entry[i];

        for (j = 0; j < ATOM_INDEX_MAX; j++)
          ad->target_atoms[i].convert_to_target[j] = converts[i][j];

#ifdef HAVE_X11
        ecore_x_selection_converter_atom_add(ad->target_atoms[i].atom,
                                             target_converters[i]);
        ecore_x_selection_converter_atom_add(ad->target_atoms[i].atom,
                                             generic_converter);
#endif
...
void
cbhmd_convert_target_atoms_deinit(Cbhmd_App_Data *ad)
{
   int i, j;
   for (i = 0; i < ATOM_INDEX_MAX; i++)
     {
        for (j = 0; j < ad->target_atoms[i].atom_cnt; j++)
          {
             if (ad->target_atoms[i].name[j])
               FREE(ad->target_atoms[i].name[j]);
          }
        if (ad->target_atoms[i].name)
          FREE(ad->target_atoms[i].name);
        if (ad->target_atoms[i].atom)
          FREE(ad->target_atoms[i].atom);
     }
}
