...
static void
_item_show_notification(Cbhmd_Cnp_Item *item, const char *msg)
{
   RET_IF(NULL == item);
   RET_IF(NULL == item->data);

   if (item->len > 0) notification_status_message_post(msg);
}

static void
_cbhmd_item_free(Cbhmd_Cnp_Item *item, Eina_Bool storage)
{
   FN_CALL();
   ...
   if (item->ad)
     {
        if (item->ad->Drawer_Item_Del_Cb) item->ad->Drawer_Item_Del_Cb(item->ad, item);
        if (storage && item->ad->Storage_Item_Del_Cb)
          item->ad->Storage_Item_Del_Cb(item->ad, item);
     }
   ...
   if (item->ad)
     {
        if (item->ad->selected_item == item) item->ad->selected_item = NULL;
     }
   FREE(item);
}

Cbhmd_Cnp_Item *
cbhmd_item_add_by_cnp_item(Cbhmd_App_Data *ad,
                           Cbhmd_Cnp_Item *item, Eina_Bool storage, Eina_Bool show_msg)
{
   ...
   if (ad && ad->Drawer_Item_Add_Cb) duplicated = ad->Drawer_Item_Add_Cb(ad, item);
   if (storage && ad && ad->Storage_Item_Add_Cb) ad->Storage_Item_Add_Cb(ad, item);
   ...
   while (ITEM_CNT_MAX < eina_list_count(ad->item_list))
     {
        ...
        _cbhmd_item_free(ditem, EINA_TRUE);
     }
   ...
   if (show_msg)
     {
        if (duplicated)
          _item_show_notification(item, S_EXIST);
        else _item_show_notification(item, S_COPY);
     }
...
static void
_item_downloaded_cb(void *data, const char *file_, int status)
{
...
static void
_item_image_name_get(char *filename, int size)
{
   time_t tim;
   struct tm now = {0};
...
static char *
_item_html_img_save_from_local(char *copied_path)
{
   ...
   _item_image_name_get(html_img_name, sizeof(html_img_name));
...
char *
_item_html_image_save(char *copied_path, Cbhmd_Cnp_Item *item)
{
   char *html_img_url = NULL;
   char html_img_name[PATH_MAX];
   int size_path = 0;
   ...
   is_network_enable = _item_get_network_state();
   ...
   if (is_network_enable)
     {
        ...
        snprintf(copied_path, size_path, "%s/%s", COPIED_DATA_STORAGE_DIR,
                 html_img_name);
        DBG("copied path = %s\n", copied_path);
        ret = ecore_file_download_full(html_img_url, copied_path, _item_downloaded_cb,
                                       NULL, item, NULL, NULL);
        if (ret) DBG("Download start");
...
#ifdef HAVE_X11
Cbhmd_Cnp_Item *cbhmd_item_add_by_data(Cbhmd_App_Data *ad, Ecore_X_Atom type, void *data, int len,
                                       Eina_Bool show_msg)
#else
Cbhmd_Cnp_Item *
cbhmd_item_add_by_data(Cbhmd_App_Data *ad, int type, void *data,
...
#endif
{
   ...
   if (item->type_index == ATOM_INDEX_TEXT)
   ...
   else if (item->type_index == ATOM_INDEX_HTML)
     {
        ...
        if (copied_path && ad->drawer->http_path)
          copied_path = _item_html_image_save(copied_path, item);
        else if (copied_path) copied_path = _item_html_img_save_from_local(copied_path);
   ...
   else if (item->type_index == ATOM_INDEX_EFL)
     {
        ...
        copied_path = cbhmd_convert_get_image_path_string(ad, ATOM_INDEX_EFL,
                                                          data);
        if (copied_path) item->img_from_markup = EINA_TRUE;
     }
   else if (item->type_index == ATOM_INDEX_IMAGE)
     {
        orig_path = data;
        file_name = ecore_file_file_get(orig_path);
        size_path = snprintf(NULL, 0, "%s/%s", COPIED_DATA_STORAGE_DIR, file_name)
           + 1;
        copied_path = MALLOC(sizeof(char) * size_path);
        if (copied_path)
          {
        ...
        if (copied_path) snprintf(copied_path, len, "%s", (char *)data);
     }
   ...
   else if (copied_path) item->gitem_style = GRID_ITEM_STYLE_IMAGE;

   item->data = data;
   item->len = len;

   item = cbhmd_item_add_by_cnp_item(ad, item, EINA_TRUE, show_msg);

   if ((item->type_index == ATOM_INDEX_IMAGE) && orig_path && copied_path)
     {
        if (!ecore_file_cp(orig_path, copied_path)) DBG("ecore_file_cp fail!");
        FREE(orig_path);
     }
...
Cbhmd_Cnp_Item *
cbhmd_item_get_by_index(Cbhmd_App_Data *ad, int index)
{
...
/*
   ...
   _cbhmd_item_get_by_data(Cbhmd_App_Data *ad, void *data, int len)
   {
   Cbhmd_Cnp_Item *item;
   Eina_List *l;
...
Cbhmd_Cnp_Item *
cbhmd_item_get_last(Cbhmd_App_Data *ad)
{
   if (!ad)
     {
        ERR("WRONG PARAMETER in %s", __func__);
...
void
cbhmd_item_delete_by_cnp_item(Cbhmd_App_Data *ad, Cbhmd_Cnp_Item *item)
{
   FN_CALL();
   ...
   _cbhmd_item_free(item, EINA_TRUE);
#ifdef HAVE_X11
   slot_property_set(ad, -1);
   cbhmd_x_handler_slot_item_count_set(ad);
#endif
...
/*
   static void
   _item_delete_by_data(Cbhmd_App_Data *ad, void *data, int len)
   {
   FN_CALL();
   if (!ad || !data)
   {
   ...
   Cbhmd_Cnp_Item *item;
   item = _cbhmd_item_get_by_data(ad, data, len);
   cbhmd_item_delete_by_cnp_item(ad, item);
   }
 */

void
cbhmd_item_delete_by_index(Cbhmd_App_Data *ad, int index)
{
   FN_CALL();
   if (!ad || eina_list_count(ad->item_list) <= index || 0 > index)
     {
   ...
   Cbhmd_Cnp_Item *item;
   item = cbhmd_item_get_by_index(ad, index);
   cbhmd_item_delete_by_cnp_item(ad, item);
}

void
cbhmd_item_clear_all(Cbhmd_App_Data *ad)
{
   FN_CALL();
   while (ad->item_list)
     {
        ...
        ad->item_list = eina_list_remove(ad->item_list, item);
        if (item) _cbhmd_item_free(item, EINA_FALSE);
     }
}

int
cbhmd_item_count_get(Cbhmd_App_Data *ad, int atom_index)
{
   int icount = 0;
   Eina_List *l;
   Cbhmd_Cnp_Item *item;
        ...
        EINA_LIST_FOREACH(ad->item_list, l, item)
          {
             if (item
                 && (item->type_index == ATOM_INDEX_TEXT
                     ...
                     || item->type_index == ATOM_INDEX_EFL)) icount++;
          }
     }
   // Get the number of image type items
   else if (atom_index == ATOM_INDEX_COUNT_IMAGE)
     {
        EINA_LIST_FOREACH(ad->item_list, l, item)
          {
             if (item && (item->type_index == ATOM_INDEX_IMAGE)) icount++;
          }
     }
