...
app_terminate(void *data)
{
   ...
   cbhmd_item_clear_all(ad);
   cbhmd_drawer_deinit(ad->drawer);

#ifdef HAVE_X11
   cbhmd_x_handler_deinit(ad->x_handler);
#endif
#ifdef HAVE_WAYLAND
   cbhmd_wl_handler_deinit(ad->wl_handler);
#endif

   cbhmd_storage_deinit(ad->storage);
...
static int
app_reset(bundle *b, void *data)
{
   return 0;
}

static int
_lang_changed_cb(void *event_info, void *data)
{
   return 0;
}
