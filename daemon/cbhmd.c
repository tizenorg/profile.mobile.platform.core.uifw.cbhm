...
static cbhmd_app_data_s *cbhmd_ad = NULL;
...
static int app_create(void *data)
{
	...
	/* init connectivity */
	ret = cbhmd_message_eldbus_init(ad);
	if (CBHM_ERROR_NONE != ret) {
		ERR("cbhmd_message_eldbus_init() Fail(%d)", ret);
		return EXIT_FAILURE;
	}
	...
	if (!(ad->drawer = cbhmd_drawer_init(ad)))
		return EXIT_FAILURE;
	...
	if (!(ad->storage = cbhmd_storage_init(ad)))
		return EXIT_FAILURE;
...
int main(int argc, char *argv[])
{
	...
	ad = calloc(1, sizeof(cbhmd_app_data_s));
	ops.data = ad;
	cbhmd_ad = ad;
