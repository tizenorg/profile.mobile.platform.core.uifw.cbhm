...
Eina_Bool
cbhmd_x_handler_send_event(Cbhmd_App_Data *ad, Ecore_X_Window xwin,
                           char *msg)
{
...
Ecore_X_Window
get_selection_owner(Cbhmd_App_Data *ad,
...
Eina_Bool
is_cbhm_selection_owner(Cbhmd_App_Data *ad,
                        Ecore_X_Selection selection)
{
   FN_CALL();
...
Eina_Bool
set_selection_owner(Cbhmd_App_Data *ad, Ecore_X_Selection selection,
                    Cbhmd_Cnp_Item *item)
{
   FN_CALL();
   if (!ad) return EINA_FALSE;
...
static Eina_Bool
selection_timer_cb(void *data)
{
   FN_CALL();
   Cbhmd_App_Data *ad = data;
   Cbhmd_X_Handler_Data *xd = ad->x_handler;
...
static Eina_Bool
_xsel_clear_cb(void *data, int type, void *event)
{
   FN_CALL();
...
static Eina_Bool
_xsel_request_cb(void *data, int type, void *event)
{
   FN_CALL();
#ifdef MDM_ENABLE
...
static void
send_convert_selection_target(Cbhmd_App_Data *ad,
        ...
        for (i = 0; i < ATOM_INDEX_MAX; i++)
          {
             ...
             for (k = 0; k < ad->target_atoms[i].atom_cnt; k++)
               {
                  if (!SAFE_STRCMP(targets_data->targets[j],
                                   ad->target_atoms[i].name[k]))
                    {
                       DBG("find matched target: %s", ad->target_atoms[i].name[k]);
                       ecore_x_selection_clipboard_request(ad->x_event_win,
                                                           ad->target_atoms[i].name[k]);
                       return;
                    }
...
static
Eina_Bool _add_selection_imagepath(Cbhmd_App_Data* ad, char *str)
{
             ...
             if (filepath)
               {
                  if (ecore_file_exists(filepath))
                    {
                       cbhmd_item_add_by_data(ad, ad->target_atoms[ATOM_INDEX_IMAGE].atom[0],
...
static void
_get_selection_data_files(Cbhmd_App_Data* ad,
...
static Eina_Bool
_xsel_notify_cb(void *data, int type, void *event)
{
   FN_CALL();
   if (!data || !event) return ECORE_CALLBACK_PASS_ON;
   ...
   if (_mdm_get_allow_clipboard())
     {
        targetAtom = ecore_x_atom_get(ev->target);
        stripstr = SAFE_STRNDUP((char * )sel_data->data, sel_data->length);
        ...
        else cbhmd_item_add_by_data(ad, targetAtom, stripstr,
...
_xclient_msg_cb(void *data, int type, void *event)
{
   ...
   if (!SAFE_STRCMP("set_owner", ev->data.b))
   ...
   else if (!SAFE_STRCMP("get count", ev->data.b))
     {
        ...
        for (i = 0; i < ATOM_INDEX_COUNT_MAX; i++)
          {
             icount = cbhmd_item_count_get(ad, i);
   ...
   /* for OSP */
   else if (!SAFE_STRNCMP("GET_ITEM", ev->data.b, 8))
     {
        ...
        Cbhmd_Cnp_Item *item = cbhmd_item_get_by_index(ad, itempos);
        if (!item)
          {
             ...
             DBG("GET ITEM ERROR msg: %s, index: %d, item count: %d",
                 ev->data.b, itempos, cbhmd_item_count_get(ad, ATOM_INDEX_COUNT_ALL));
          }
        else
          {
             ecore_x_window_prop_property_set(
                ev->win,
                xd->atomCBHM_ITEM,
                ad->target_atoms[item->type_index].atom[0],
                8,
                item->data,
                item->len);
             DBG("GET ITEM index: %d, item type: %d, item data: %s, item->len: %d",
                 itempos, ad->target_atoms[item->type_index].atom[0],
   ...
   else if (!SAFE_STRNCMP("SET_ITEM", ev->data.b, 8))
     {
        ...
        free(format_name);
        cbhmd_item_add_by_data(ad, format, item_data, SAFE_STRLEN((char *)item_data) + 1, EINA_TRUE);
     }
   else if (!SAFE_STRNCMP("DEL_ITEM", ev->data.b, 8))
     {
        ...
        cbhmd_item_delete_by_index(ad, itempos);
     }
...
static Eina_Bool
_xfocus_out_cb(void *data, int type, void *event)
{
   FN_CALL();
   if (!data || !event) return ECORE_CALLBACK_PASS_ON;
...
static Eina_Bool
_xproperty_notify_cb(void *data, int type, void *event)
{
   if (!data || !event) return ECORE_CALLBACK_PASS_ON;
             ...
             if (filepath && ecore_file_exists(filepath))
               {
                  ...
                  cbhmd_item_add_by_data(ad, ad->target_atoms[ATOM_INDEX_IMAGE].atom[0], filepath, SAFE_STRLEN(filepath) + 1, EINA_FALSE);
               }
             else FREE(filepath);
          }
     }
...
static Eina_Bool
_xwin_destroy_cb(void *data, int type, void *event)
{
   FN_CALL();
   if (!data || !event) return ECORE_CALLBACK_PASS_ON;
...
void
slot_property_set(Cbhmd_App_Data *ad, int index)
{
   Cbhmd_X_Handler_Data *xd = ad->x_handler;
   ...
   if (index < 0)
             ...
             if (item)
               {
                  ecore_x_window_prop_property_set(
                     ...
                     item->len);
                  DBG("GET ITEM index: %d, item type: %d, item data: %s, item->len: %d",
                      i, ad->target_atoms[item->type_index].atom[0],
                      (char *)item->data, item->len);
               }
#endif
             i++;
   ...
   else if (index < ITEM_CNT_MAX)
     {
        ...
        Cbhmd_Cnp_Item *item = cbhmd_item_get_by_index(ad, index);
        if (!item)
          {
             Ecore_X_Atom itemtype = ecore_x_atom_get("CBHM_ERROR");
             char error_buf[] = "OUT OF BOUND";
             ...
             DBG("CBHM Error: index: %d, item count: %d",
                 index, cbhmd_item_count_get(ad, ATOM_INDEX_COUNT_ALL));
          }
        else
          {
             ecore_x_window_prop_property_set(
                ...
                item->len);
             DBG("GET ITEM index: %d, item type: %d, item data: %s, item->len: %d",
                 index, ad->target_atoms[item->type_index].atom[0],
                 (char *)item->data, item->len);
          }
#endif
     }
...
void
cbhmd_x_handler_slot_item_count_set(Cbhmd_App_Data *ad)
{
   Cbhmd_X_Handler_Data *xd = ad->x_handler;
   int i, icount;
   char countbuf[10];
...
void
slot_selected_item_set(Cbhmd_App_Data *ad)
{
   Cbhmd_X_Handler_Data *xd = ad->x_handler;
...
int
cbhmd_x_handler_init(Cbhmd_App_Data *ad)
{
   FN_CALL();
...
void
cbhmd_x_handler_deinit(Cbhmd_X_Handler_Data *xd)
{
