                              ...
                              Cbhmd_Cnp_Item *item)
{
   ...
   Eina_Bool (*selection_func)(Ecore_X_Window win, const void *data,
                               int size) = NULL;
...
static Eina_Bool _xsel_clear_cb(void *data, int type, void *event)
{
   ...
   if (is_cbhm_selection_owner(ad, ev->selection))
     return EINA_TRUE;
...
static Eina_Bool _xsel_request_cb(void *data, int type, void *event)
{
   ...
   if (ev->selection == ECORE_X_ATOM_SELECTION_CLIPBOARD)
     ...
     if (ev->selection == ECORE_X_ATOM_SELECTION_SECONDARY)
...
{
   if (!ad || !str)
     return EINA_FALSE;DBG("get FILE: %s", str);
