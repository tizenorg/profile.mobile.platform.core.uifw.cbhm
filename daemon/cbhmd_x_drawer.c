...
int cbhmd_x_drawer_effect_and_focus_set(cbhmd_app_data_s *ad)
{
	FN_CALL();
	...
	ATOM_WINDOW_EFFECT_ENABLE = ecore_x_atom_get(
	...
	ecore_x_window_prop_property_set(x_root_win,
			ecore_x_atom_get("CBHM_ELM_WIN"), ECORE_X_ATOM_WINDOW, 32,
			&x_main_win, 1);
	ecore_x_flush();
...
{
	Ecore_X_Window x_transient_win = ecore_x_icccm_transient_for_get(
			x_main_win);
...
int cbhmd_x_drawer_event_window_create(cbhmd_app_data_s *ad)
{
	...
	if (ad->x_disp) {
		...
		if (!clipboard_manager) {
			...
			if (ad->x_root_win) {
				ad->x_event_win = ecore_x_window_new(ad->x_root_win, 0, 0, 19,
						19);
				DBG("x_event_win: 0x%x", ad->x_event_win);
				if (ad->x_event_win) {
					XSetSelectionOwner(ad->x_disp, clipboard_manager_atom,
							ad->x_event_win, CurrentTime);
					Ecore_X_Window clipboard_manager = XGetSelectionOwner(
							ad->x_disp, clipboard_manager_atom);
