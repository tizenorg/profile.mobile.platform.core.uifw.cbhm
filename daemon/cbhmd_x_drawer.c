...
void
cbhmd_x_drawer_focus_set(Ecore_X_Window x_main_win, Eina_Bool enable)
{
   FN_CALL();
...
void
cbhmd_x_drawer_set_transient(Ecore_X_Window transient_win,
...
void
cbhmd_x_drawer_unset_transient(Ecore_X_Window x_main_win)
{
...
void
cbhmd_x_drawer_event_window_set_title(Ecore_X_Window x_event_win,
...
int
cbhmd_x_drawer_event_window_create(Cbhmd_App_Data *ad)
{
   Ecore_X_Atom clipboard_manager_atom;
   Ecore_X_Window clipboard_manager;
   ...
   clipboard_manager_atom = XInternAtom(ad->x_disp, CBHMD_ATOM_CLIPBOARD_MANAGER_NAME, False);
   clipboard_manager = XGetSelectionOwner(ad->x_disp, clipboard_manager_atom);

   DBG("clipboard_manager_window: 0x%x", clipboard_manager);

   if (clipboard_manager)
     {
        DBG("clipboard_manager already exist");
        return CBHM_ERROR_ALREADY_IN_USE;
     }
...
Ecore_X_Window cbhmd_x_drawer_isf_ise_window_get()
{
   ...
   if (ret && num) memcpy(&x_isf_control_win, buf, sizeof(Ecore_X_Window));
   if (buf) free(buf);
   if (!x_isf_control_win) return 0;

   ret = ecore_x_window_prop_property_get(x_isf_control_win, x_atom_isf_ise,
                                          ECORE_X_ATOM_WINDOW, 0, &buf, &num);
   if (ret && num) memcpy(&x_isf_ise_win, buf, sizeof(Ecore_X_Window));
   if (buf) free(buf);
