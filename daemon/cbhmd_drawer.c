...
static void _drawer_grid_item_btn_clicked_cb(void *data, Evas_Object *obj,
                                             void *event_info);
static void _cbhmd_drawer_evas_object_focus_set(Evas_Object *obj,
                                                Eina_Bool enable);

static Evas_Event_Flags
_drawer_flick_end_cb(void *data, void *event_info)
{
   ...
   if (y_diff > 0 && y_diff > abs(x_diff)) cbhmd_drawer_hide(ad);

   return EVAS_EVENT_FLAG_NONE;
}

static void
_cbhmd_drawer_textonly_mode_set(Cbhmd_Drawer_Data *dd)
{
...
void
cbhmd_drawer_text_only_mode_set(Cbhmd_App_Data *ad, Eina_Bool textonly)
{
   FN_CALL();
   Cbhmd_Drawer_Data *dd = ad->drawer;
   if (dd->paste_text_only != textonly) dd->paste_text_only = textonly;
...
Eina_Bool
cbhmd_drawer_text_only_mode_get(Cbhmd_App_Data *ad)
{
   Cbhmd_Drawer_Data *dd = ad->drawer;
...
static int
_cbhmd_drawer_edj_load(Cbhmd_Drawer_Data *dd, const char *file,
                       const char *group)
{
...
static void
_drawer_gengrid_select_cb(void *data, Evas_Object *obj, void *event)
{
   ...
   if (dd->del_btn_clicked)
     {
        cbhmd_item_delete_by_cnp_item(ad, item);
...
static Eina_Bool
_drawer_keydown_cb(void *data, int type, void *event)
{
...
static char *
_drawer_title_close_btn_access_type_cb(void *data, Evas_Object *obj)
{
...
static char *
_drawer_title_close_btn_access_info_cb(void *data, Evas_Object *obj)
{
   Cbhmd_App_Data *ad = (Cbhmd_App_Data *)data;
   if (!ad) return NULL;
...
static char *
_drawer_title_delete_btn_access_type_cb(void *data, Evas_Object *obj)
{
   Cbhmd_App_Data *ad = (Cbhmd_App_Data *)data;
   if (!ad) return NULL;
...
static char *
_drawer_title_delete_btn_access_info_cb(void *data, Evas_Object *obj)
{
   Cbhmd_App_Data *ad = (Cbhmd_App_Data *)data;
   if (!ad) return NULL;
...
static char *
_drawer_title_delete_all_btn_access_type_cb(void *data, Evas_Object *obj)
{
   Cbhmd_App_Data *ad = (Cbhmd_App_Data *)data;
   if (!ad) return NULL;
...
static char *
_drawer_title_delete_all_btn_access_info_cb(void *data, Evas_Object *obj)
{
   Cbhmd_App_Data *ad = (Cbhmd_App_Data *)data;
   if (!ad) return NULL;
...
static void
_drawer_title_close_btn_access_activate_cb(void *data,
...
static void
_drawer_title_delete_btn_access_activate_cb(void *data,
                                            Evas_Object *obj, Elm_Object_Item *item)
{
   Cbhmd_App_Data *ad = (Cbhmd_App_Data *)data;
   if (!ad) return;
...
static void
_drawer_title_delete_all_btn_access_activate_cb(void *data,
                                                Evas_Object *obj, Elm_Object_Item *item)
{
   FN_CALL();
   int ret;
   ...
   if ((cbhmd_item_count_get(ad, ATOM_INDEX_COUNT_ALL) - dd->locked_item_count) != 0)
     {
...
static Eina_Bool
_drawer_item_add(Cbhmd_App_Data *ad, Cbhmd_Cnp_Item *item)
{
   ...
   EINA_LIST_FOREACH_SAFE(itemlist, l, l_next, gitem_data)
     {
        if ((gitem_data->type_index == item->type_index)
            && (!SAFE_STRCMP(item->data, gitem_data->data)))
          {
             ...
             cbhmd_item_delete_by_cnp_item(ad, gitem_data);
             break;
          }
     }

   if (item->locked) dd->locked_item_count++;

   if (item->gitem_style == GRID_ITEM_STYLE_TEXT)
     item->gitem = elm_gengrid_item_prepend(dd->gengrid, &dd->gic_text, item,
                                            _drawer_gengrid_select_cb, item);
   else if (item->gitem_style == GRID_ITEM_STYLE_IMAGE)
     item->gitem = elm_gengrid_item_prepend(dd->gengrid, &dd->gic_image, item,
                                            _drawer_gengrid_select_cb, item);
   else item->gitem = elm_gengrid_item_prepend(dd->gengrid, &dd->gic_combined,
                                               item, _drawer_gengrid_select_cb, item);

   return duplicated;
}

static Eina_Bool
_drawer_item_del(Cbhmd_App_Data *ad, Cbhmd_Cnp_Item *item)
{
   if (item->gitem)
     {
...
static char *
_drawer_grid_text_get(void *data, Evas_Object *obj, const char *part)
{
   FN_CALL();
   Cbhmd_Cnp_Item *item = data;
...
static Evas_Object *
_drawer_grid_content_get(void *data, Evas_Object *obj, const char *part)
{
   Cbhmd_Cnp_Item *item = data;
   ...
   if (!SAFE_STRCMP(part, "delbtn/img")) /* text */
     {
        ...
        evas_object_smart_callback_add(btn, "clicked",
                                       _drawer_grid_item_btn_clicked_cb, item);
        return btn;
     }
   else return NULL;
}
...
static Evas_Object *
_drawer_grid_image_content_get(void *data, Evas_Object *obj, const char *part)
{
   Cbhmd_Cnp_Item *item = data;
   Cbhmd_App_Data *ad;
   ...
   else if (!SAFE_STRCMP(part, "delbtn/img"))
     { /* text */
        Evas_Object *btn = elm_button_add(obj);
        elm_object_style_set(btn, "delete_icon");
        evas_object_propagate_events_set(btn, EINA_FALSE);
...
static Evas_Object *
_drawer_grid_combined_content_get(void *data, Evas_Object *obj, const char *part)
{
   Cbhmd_Cnp_Item *item = data;
   Cbhmd_App_Data *ad;
   Cbhmd_Drawer_Data *dd;
...
static void
_drawer_grid_realized_cb(void *data, Evas_Object *obj, void *event_info)
{
...
static void
_drawer_ok_btn_clicked_cb(void *data, Evas_Object *obj, void *event_info)
{
   Cbhmd_App_Data *ad = data;
   Cbhmd_Drawer_Data *dd = ad->drawer;
   Elm_Object_Item *gitem = elm_gengrid_first_item_get(dd->gengrid);
   ...
   while (gitem)
     {
        ...
        if (!gitem_data->locked) cbhmd_item_delete_by_cnp_item(ad, gitem_data);
     }

   if (cbhmd_item_count_get(ad, ATOM_INDEX_COUNT_ALL) == 0)
     cbhmd_drawer_hide(ad);
   else
     {
        dd->popup_activate = EINA_FALSE;
...
static void
_drawer_cancel_btn_clicked_cb(void *data, Evas_Object *obj, void *event_info)
{
   Cbhmd_App_Data *ad = data;
   Cbhmd_Drawer_Data *dd = ad->drawer;
...
static void
_drawer_grid_item_btn_clicked_cb(void *data, Evas_Object *obj, void *event_info)
{
   Cbhmd_Cnp_Item *item = data;
   Cbhmd_App_Data *ad = item->ad;
   Cbhmd_Drawer_Data *dd = ad->drawer;

   if (dd->anim_status != STATUS_NONE) return;

   cbhmd_item_delete_by_cnp_item(ad, item);
}

static void
_drawer_layout_clicked_cb(void *data, Evas_Object *obj, const char *emission, const char *source)
{
   int ret;
   Cbhmd_App_Data *ad = data;

   if (ad->drawer->anim_status != STATUS_NONE) return;
   ...
   else if (!SAFE_STRNCMP(source, EDJE_DELETE_MODE_PREFIX,
                          SAFE_STRLEN(EDJE_DELETE_MODE_PREFIX)))
     {
        cbhmd_drawer_delete_mode_set(ad, !delete_mode);
     }
   else if (!SAFE_STRNCMP(source, EDJE_DELETE_ALL_BTN_PART_PREFIX,
...
static int
_cbhmd_drawer_popup_create(Cbhmd_App_Data *ad)
{
   FN_CALL();
   int w = 0, h = 0;
   Evas_Object *btn1;
   ...
   if (dd->popup_activate == EINA_TRUE) return CBHM_ERROR_NONE;
   ...
   evas_object_smart_callback_add(btn1, "clicked",
                                  _drawer_cancel_btn_clicked_cb, ad);
   ...
   evas_object_smart_callback_add(btn2, "clicked", _drawer_ok_btn_clicked_cb,
                                  ad);
...
static void
_cbhmd_drawer_evas_object_focus_set(Evas_Object *obj,
                                    Eina_Bool enable)
{
#ifdef HAVE_X11
...
void
cbhmd_drawer_focus_set(Cbhmd_App_Data *ad, Eina_Bool enable)
{
   Cbhmd_Drawer_Data *dd;
...
static Evas_Object *
_cbhmd_drawer_win_add(Cbhmd_Drawer_Data *dd, const char *name)
{
...
static void
_cbhmd_drawer_set_geometry(Cbhmd_App_Data *ad)
{
   FN_CALL();
   Cbhmd_Drawer_Data *dd = ad->drawer;
   Evas_Coord x, y, w, h;
...
void
cbhmd_drawer_set_rotation(Cbhmd_App_Data *ad)
{
   FN_CALL();
   Cbhmd_Drawer_Data *dd = ad->drawer;
   int angle = elm_win_rotation_get(dd->main_win);
...
static Eina_Bool clipdrawer_anim_effect(Cbhmd_App_Data *ad, Anim_Status atype)
{
   FN_CALL();
   Cbhmd_Drawer_Data *dd = ad->drawer;
...
static Eina_Bool
_cbhmd_drawer_gengrid_show_timer_cb(void *data)
{
...
void
_drawer_wm_rotation_changed_cb(void *data, Evas_Object * obj, void *event)
{
   if (!data) return;
...
void
cbhmd_drawer_delete_mode_set(Cbhmd_App_Data* ad, Eina_Bool del_mode)
{
   Cbhmd_Drawer_Data *dd = ad->drawer;
   Elm_Object_Item *gitem = elm_gengrid_first_item_get(dd->gengrid);
   Cbhmd_Cnp_Item *item = NULL;
...
int
cbhmd_drawer_event_window_create(Cbhmd_App_Data *ad)
{
   FN_CALL();

   if (!ad) return CBHM_ERROR_INVALID_PARAMETER;
...
void
cbhmd_drawer_event_window_set_title(Cbhmd_App_Data *ad)
{
#ifdef HAVE_X11
...
int
cbhmd_drawer_display_init(Cbhmd_App_Data *ad)
{
#ifdef HAVE_X11
...
void
cbhmd_drawer_show(Cbhmd_App_Data* ad)
{
   FN_CALL();
   Cbhmd_Drawer_Data *dd = ad->drawer;
#ifdef HAVE_X11
   ...
   if (dd->main_layout)
     {
        int item_count = cbhmd_item_count_get(ad, ATOM_INDEX_COUNT_ALL)
   ...
   if (dd->main_win)
     {
        ...
        evas_object_smart_callback_add(dd->main_win, "wm,rotation,changed",
                                       _drawer_wm_rotation_changed_cb, ad);
        cbhmd_drawer_delete_mode_set(ad, EINA_FALSE);
        cbhmd_drawer_set_rotation(ad);
        evas_object_show(dd->main_win);
...
static Eina_Bool
cbhmd_drawer_hide_timer_cb(void *data)
{
   FN_CALL();
   Cbhmd_App_Data *ad = (Cbhmd_App_Data *)data;
   Cbhmd_Drawer_Data *dd = ad->drawer;
...
void
cbhmd_drawer_hide(Cbhmd_App_Data* ad)
{
   FN_CALL();
   Cbhmd_Drawer_Data *dd = ad->drawer;
#ifdef HAVE_X11
...
Cbhmd_Drawer_Data *
cbhmd_drawer_init(Cbhmd_App_Data *ad)
{
   FN_CALL();
   Evas_Object* ly;
   const char *data;
   ...
   elm_access_info_cb_set(access_obj, ELM_ACCESS_TYPE,
                          _drawer_title_close_btn_access_type_cb, ad);
   elm_access_info_cb_set(access_obj, ELM_ACCESS_INFO,
                          _drawer_title_close_btn_access_info_cb, ad);
   elm_access_activate_cb_set(access_obj,
                              _drawer_title_close_btn_access_activate_cb, ad);
   ...
   elm_access_info_cb_set(access_obj, ELM_ACCESS_TYPE,
                          _drawer_title_delete_btn_access_type_cb, ad);
   elm_access_info_cb_set(access_obj, ELM_ACCESS_INFO,
                          _drawer_title_delete_btn_access_info_cb, ad);
   elm_access_activate_cb_set(access_obj,
                              _drawer_title_delete_btn_access_activate_cb, ad);
   ...
   elm_access_info_cb_set(access_obj, ELM_ACCESS_TYPE,
                          _drawer_title_delete_all_btn_access_type_cb, ad);
   elm_access_info_cb_set(access_obj, ELM_ACCESS_INFO,
                          _drawer_title_delete_all_btn_access_info_cb, ad);
   elm_access_activate_cb_set(access_obj,
                              _drawer_title_delete_all_btn_access_activate_cb, ad);
   ...
   elm_gesture_layer_cb_set(dd->gesture_layer, ELM_GESTURE_N_FLICKS,
                            ELM_GESTURE_STATE_END, _drawer_flick_end_cb, ad);
...
void
cbhmd_drawer_deinit(Cbhmd_Drawer_Data *dd)
{
#ifdef HAVE_X11
...
#endif
   evas_object_del(dd->main_win);
   if (dd->anim_timer) ecore_timer_del(dd->anim_timer);
   if (dd->keydown_handler) ecore_event_handler_del(dd->keydown_handler);
   if (dd->event_rect) evas_object_del(dd->event_rect);
   if (dd->gesture_layer)
     {
        elm_gesture_layer_cb_set(dd->gesture_layer, ELM_GESTURE_N_FLICKS,
