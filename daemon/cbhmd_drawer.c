...
static int __cbhmd_drawer_popup_create(cbhmd_app_data_s *ad);
static void __drawer_grid_item_btn_clicked_cb(void *data, Evas_Object *obj,
		void *event_info);
static void __cbhmd_drawer_evas_object_focus_set(Evas_Object *obj,
		Eina_Bool enable);
...
static void __cbhmd_drawer_textonly_mode_set(cbhmd_drawer_data_s *dd)
{
	...
	while (gitem) {
		...
		if (item->gitem_style == GRID_ITEM_STYLE_COMBINED) {
			entry_text = cbhmd_convert_get_entry_string(item->ad,
...
void cbhmd_drawer_text_only_mode_set(cbhmd_app_data_s *ad, Eina_Bool textonly)
{
	...
	__cbhmd_drawer_textonly_mode_set(dd);
}
...
static int __cbhmd_drawer_edj_load(cbhmd_drawer_data_s *dd, const char *file,
		const char *group)
{
...
static char *
__drawer_title_close_btn_access_type_cb(void *data,
										  Evas_Object *obj)
{
	cbhmd_app_data_s *ad = (cbhmd_app_data_s *)data;
	if (!ad) return NULL;
...
static char *
__drawer_title_close_btn_access_info_cb(void *data,
										  Evas_Object *obj)
{
	cbhmd_app_data_s *ad = (cbhmd_app_data_s *)data;
	if (!ad) return NULL;
...
static char *
__drawer_title_delete_btn_access_type_cb(void *data,
											Evas_Object *obj)
{
	cbhmd_app_data_s *ad = (cbhmd_app_data_s *)data;
	if (!ad) return NULL;
...
static char *
__drawer_title_delete_btn_access_info_cb(void *data,
											Evas_Object *obj)
{
	cbhmd_app_data_s *ad = (cbhmd_app_data_s *)data;
	if (!ad) return NULL;
...
static char *
__drawer_title_delete_all_btn_access_type_cb(void *data,
												 Evas_Object *obj)
{
	cbhmd_app_data_s *ad = (cbhmd_app_data_s *)data;
	if (!ad) return NULL;
...
static char *
__drawer_title_delete_all_btn_access_info_cb(void *data,
												 Evas_Object *obj)
{
	cbhmd_app_data_s *ad = (cbhmd_app_data_s *)data;
	if (!ad) return NULL;
...
static void
__drawer_title_close_btn_access_activate_cb(void *data,
												Evas_Object *obj,
...
static void
__drawer_title_delete_btn_access_activate_cb(void *data,
												 Evas_Object *obj,
												 Elm_Object_Item *item)
{
	cbhmd_app_data_s *ad = (cbhmd_app_data_s *)data;
...
static void
__drawer_title_delete_all_btn_access_activate_cb(void *data,
													  Evas_Object *obj,
													  Elm_Object_Item *item)
{
	FN_CALL();
	...
	if ((item_count_get(ad, ATOM_INDEX_COUNT_ALL) - dd->locked_item_count)
			!= 0) {
		ret = __cbhmd_drawer_popup_create(ad);
		if (CBHM_ERROR_NONE != ret) {
			ERR("__cbhmd_drawer_popup_create() Fail(%d)", ret);
			return;
		}
	}
}

static int __cbhmd_drawer_effect_and_focus_set(cbhmd_app_data_s *ad)
{
	FN_CALL();
	int ret;
...
static Eina_Bool __drawer_item_add(cbhmd_app_data_s *ad, cbhmd_cnp_item_s *item)
{
	Eina_Bool duplicated = EINA_FALSE;
	cbhmd_drawer_data_s *dd = ad->drawer;
	cbhmd_cnp_item_s *gitem_data = NULL;
	Eina_List *l = NULL;
	...
	if (item->gitem_style == GRID_ITEM_STYLE_TEXT)
		...
		item->gitem = elm_gengrid_item_prepend(dd->gengrid, &dd->gic_combined,
				item, __drawer_gengrid_select_cb, item);

	return duplicated;
}

static Eina_Bool __drawer_item_del(cbhmd_app_data_s *ad, cbhmd_cnp_item_s *item)
{
	if (item->gitem) {
...
static char *__drawer_grid_text_get(void *data, Evas_Object *obj, const char *part)
{
	FN_CALL();
	cbhmd_cnp_item_s *item = data;
...
static Evas_Object *__drawer_grid_content_get(void *data, Evas_Object *obj, const char *part)
{
	cbhmd_cnp_item_s *item = data;
		...
		void *event_info)
{
	...
	while (gitem) {
		cbhmd_cnp_item_s *gitem_data = elm_object_item_data_get(gitem);
		gitem = elm_gengrid_item_next_get(gitem);
...
static int __cbhmd_drawer_popup_create(cbhmd_app_data_s *ad)
{
	...
	__cbhmd_drawer_evas_object_focus_set(dd->popup_win, EINA_FALSE);
...
static Evas_Object* __cbhmd_drawer_win_add(cbhmd_drawer_data_s *dd, const char *name)
{
...
static void __cbhmd_drawer_set_geometry(cbhmd_app_data_s *ad)
{
	FN_CALL();
	cbhmd_drawer_data_s *dd = ad->drawer;
	Evas_Coord x, y, w, h;
...
void cbhmd_drawer_set_rotation(cbhmd_app_data_s *ad)
{
	...
	__cbhmd_drawer_set_geometry(ad);
}
/*
static Eina_Bool _get_anim_pos(cbhmd_drawer_data_s *dd, int *sp, int *ep)
{
	if (!sp || !ep)
		return EINA_FALSE;
...
static Eina_Bool _do_anim_delta_pos(cbhmd_drawer_data_s *dd, int sp, int ep, int ac, int *dp)
{
	if (!dp)
		return EINA_FALSE;
...
static void stop_animation(cbhmd_app_data_s *ad)
{
	FN_CALL();
	cbhmd_drawer_data_s *dd = ad->drawer;
...
static Eina_Bool clipdrawer_anim_effect(cbhmd_app_data_s *ad, AnimStatus atype)
{
	FN_CALL();
	cbhmd_drawer_data_s *dd = ad->drawer;
...
static Eina_Bool __cbhmd_drawer_gengrid_show_timer_cb(void *data)
{
	cbhmd_drawer_data_s *dd = data;
...
void cbhmd_drawer_delete_mode_set(cbhmd_app_data_s* ad, Eina_Bool del_mode)
{
	...
	while (gitem) {
		...
		if (!item->locked) {
			if (delete_mode)
				elm_object_item_signal_emit(gitem, "elm,state,show,delbtn",
						"elm");
			else
				elm_object_item_signal_emit(gitem, "elm,state,hide,delbtn",
						"elm");
		}
...
//		cbhmd_wl_drawer_set_transient(dd->wl_main_win, wl_transient_win);
		...
		ecore_timer_add(0.125, __cbhmd_drawer_gengrid_show_timer_cb, dd);
	}
...
cbhmd_drawer_data_s* cbhmd_drawer_init(cbhmd_app_data_s *ad)
{
	...
	cbhmd_drawer_data_s *dd = calloc(1, sizeof(cbhmd_drawer_data_s));
	...
	if (!(dd->main_win = __cbhmd_drawer_win_add(dd, APPNAME))) {
		ERR("__cbhmd_drawer_win_add() Fail");
		free(dd);
		return NULL;
	}
#ifdef HAVE_X11
	...
	ret = __cbhmd_drawer_effect_and_focus_set(ad);
	if (CBHM_ERROR_NONE != ret) {
		ERR("__cbhmd_drawer_effect_and_focus_set() Fail");
		free(dd);
		return NULL;
	}

	/* edj setting */
	ret = __cbhmd_drawer_edj_load(dd, APP_EDJ_FILE, GRP_MAIN);
	if (CBHM_ERROR_NONE != ret) {
		ERR("__cbhmd_drawer_edj_load() Fail");
